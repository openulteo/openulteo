...
int __guac_handle_ukbrdr(guac_client* client, guac_instruction* instruction) {
    if (client->ukbrdr_handler)
        return client->ukbrdr_handler(
            client,
            instruction->argv[0] /* data */
        );
    return 0;
