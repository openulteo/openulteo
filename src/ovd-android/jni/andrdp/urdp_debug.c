 ...
 */
typedef struct urdp_stdio_debug_t {
	int stdoutPipe[2];
	int stderrPipe[2];
	boolean haltStdioConverter;
	pthread_t stdioConverterThread;
} urdp_stdio_debug;
...
static boolean readAndLog(int fd, BufferedData* data, boolean tag);
 ...
 * Returns immediately.
 */
boolean urdp_setup_stdio(void) {
	urdp_stdio_debug* pipeStorage;

	pipeStorage = (urdp_stdio_debug*) malloc(sizeof(urdp_stdio_debug));
	if (pipeStorage == NULL)
		return false;

	pipeStorage->haltStdioConverter = false;

	if (pipe(pipeStorage->stdoutPipe) != 0) {
		log_warning("pipe failed: %s", strerror(errno));
		return false;
	}
	...
	if (dup2(pipeStorage->stdoutPipe[1], kFilenoStdout) != kFilenoStdout) {
		log_warning("dup2(1) failed: %s", strerror(errno));
	...
	if (dup2(pipeStorage->stderrPipe[1], kFilenoStderr) != kFilenoStderr) {
		log_warning("dup2(2) failed: %d %s", errno, strerror(errno));
		close(pipeStorage->stdoutPipe[1]);
		free(pipeStorage);
		return false;
	}
	 ...
	 * Create the thread.
	 */
	if (pthread_create(&pipeStorage->stdioConverterThread, 0, stdioConverterThreadStart, pipeStorage) != 0) {
		free(pipeStorage);
		return false;
	}
 ...
 */
static void* stdioConverterThreadStart(void* arg) {
	urdp_stdio_debug* pipeStorage = (urdp_stdio_debug*) arg;
	 ...
	 */
	while (!pipeStorage->haltStdioConverter) {
		//ssize_t actual;
		fd_set readfds;
		int maxFd, fdCount;
		...
		if (fdCount < 0) {
			...
			log_debug("Got EINTR, ignoring");
		} else if (fdCount == 0) {
			log_debug("WEIRD: select returned zero");
		} else {
			boolean err = false;
			if (FD_ISSET(pipeStorage->stdoutPipe[0], &readfds)) {
				err |= !readAndLog(pipeStorage->stdoutPipe[0], stdoutData, true);
			}
			if (FD_ISSET(pipeStorage->stderrPipe[0], &readfds)) {
				err |= !readAndLog(pipeStorage->stderrPipe[0], stderrData, false);
			}

			/* probably EOF; give up */
			if (err) {
				log_warning("stdio converter got read error; shutting it down\n");
				break;
			}
		}
	}
	...
	for (i = data->count; i > 0; i--, cp++) {
		if (*cp == '\n' || (*cp == '\r' && i != 0 && *(cp + 1) != '\n')) {
			*cp = '\0';
			...
			if (tag) {
				log_info("STDOUT: %s", start);
			} else {
				log_error("STDERR: %s", start);
			}
			start = cp + 1;
		}
	}
