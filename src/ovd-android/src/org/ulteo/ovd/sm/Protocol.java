...
public class Protocol {
    ...
    public static final String[] settingsNames = { NAME_DESKTOP_ICONS, SESSION_MODE_PERSISTENT, NAME_USER_EXPERIENCE,
	    NAME_ATTR_MULTIMEDIA, NAME_ATTR_PRINTERS, NAME_ATTR_DRIVES, NAME_ATTR_RDP_BPP, };
	...
	if (name.equalsIgnoreCase(NAME_DESKTOP_ICONS)) {
		...
		properties.setDesktopIcons(val > 0);
			...
			if (Config.DEBUG)
				Log.d(Config.TAG, "Failed to parse value '" + value + "' (name: " + NAME_DESKTOP_ICONS + ")");
	    }
	}
	if (name.equalsIgnoreCase(SESSION_MODE_PERSISTENT)) {
		...
		properties.setPersistent(val > 0);
			...
			if (Config.DEBUG)
				Log.d(Config.TAG, "Failed to parse value '" + value + "' (name: " + SESSION_MODE_PERSISTENT + ")");
	    }
	}
	if (name.equalsIgnoreCase(NAME_USER_EXPERIENCE)) {
		...
		properties.setDesktopEffects(val > 0);
			...
			if (Config.DEBUG)
				Log.d(Config.TAG, "Failed to parse value '" + value + "' (name: " + NAME_USER_EXPERIENCE + ")");
	    }
	}
	if (name.equalsIgnoreCase(NAME_ATTR_MULTIMEDIA)) {
		...
		properties.setMultimedia(val > 0);
			...
			if (Config.DEBUG)
				Log.d(Config.TAG, "Failed to parse value '" + value + "' (name: " + NAME_ATTR_MULTIMEDIA + ")");
	    }
	}
	...
	if (name.equalsIgnoreCase(NAME_ATTR_PRINTERS)) {
		...
		properties.setPrinters(val > 0);
			...
			if (Config.DEBUG)
				Log.d(Config.TAG, "Failed to parse value '" + value + "' (name: " + NAME_ATTR_PRINTERS + ")");
	    }
	}
